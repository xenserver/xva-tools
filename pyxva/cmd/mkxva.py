...
#  http://www-archive.xenproject.org/files/xva/xva.py
...
def make_xva(output_path, sparse, ova_xml_path, disk_path, disk_reference):
    ...
    ova_content = str(ova_file.read()).replace(
        "@VDI_SIZE@", str(os.stat(disk_path).st_size))
    ova_file.close()
...
def main():
    ...
    parser.add_argument(
        '--ova_xml_path',
        required=True,
        help='Path of the ova.xml for the xva')
    parser.add_argument(
        '--disk_path', required=True,  help='Path of the disk for the xva')
    parser.add_argument(
        '--disk_reference',
        required=True,
        help='The XVA reference to the disk')
    args = parser.parse_args()
    make_xva(args.output_path, True, args.ova_xml_path, args.disk_path,
             args.disk_reference)
